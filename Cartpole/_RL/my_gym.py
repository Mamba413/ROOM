...
class GymEval():
    """ make the online carepole env suitable for offline OPE and OPO
    NOW: already merged into the env
    
    """
    def __init__(self, random_pi = False, env = None):
    ...
    def eval_policy(self, pi, gamma, init_states = None, rep = 1000):
        ...
        for i in range(rep):
            ...
            Vs.append(V)
        Vs = np.clip(Vs, np.quantile(Vs, [0.01]), np.quantile(Vs, [0.99]))
        V_true = np.mean(Vs)
    ...
    def simu_trajs_para(self, pi, rep = 100, T = 1000
        ...
        """
        ######## 
        Ss = self.env.reset_multiple(rep)
        Ss = Ss.T
        if init_states is not None:
            self.env.states = init_states.T
            Ss = init_states
        ...
        for t in range(T):
            ...
            if self.random_pi:
                ...
                As = pi.get_A(Ss)
            SSs, Rs, _, _ = self.env.step_multiple(As)
            for i in range(rep):
